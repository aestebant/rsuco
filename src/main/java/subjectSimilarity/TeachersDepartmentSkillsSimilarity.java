 ...
 */
public class TeachersDepartmentSkillsSimilarity implements ItemSimilarity, IConfiguration {
	 ...
	 * 
	 */
	public TeachersDepartmentSkillsSimilarity(DataModel teaching, DataModel skills, DataModel departments, Configuration config) {
		configure(config);
	...
	public double itemSimilarity(long subject1, long subject2) throws TasteException {
		...
		double simSkill = skillSim.userSimilarity(subject1, subject2);
		
		if(Double.isNaN(simTeaching))
			simTeaching = .0;
		if(Double.isNaN(simSkill))
			simSkill = .0;
		
		double similarity = simTeaching * wTeaching + simSkill * (1 - wTeaching);
		
		similarity += bonusDepartment(subject1, subject2);
	...
	public void configure(Configuration config) {
		try {
			...
			this.iSkillSim = (Class<? extends UserSimilarity>) Class.forName(config.getString("skillSimilarity"));
		} catch (ClassNotFoundException e) {
			e.printStackTrace();
		}
		this.wTeaching = config.getDouble("teachingWeight");
	}
